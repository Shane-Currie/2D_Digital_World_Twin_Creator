...
- `preview_ready` means the shared functional town preview is launchable, not that every "Generational Australian Survival v1.3" system is complete. Keep `preview_limitations` accurate and do not mark Node-only imports playable before Godot has generated their navigation data.
