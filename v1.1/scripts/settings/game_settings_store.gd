...
static func load_from_town(town_directory: String) -> Dictionary:
	...
	var saved_skin_tones: Dictionary = parsed.get("skin_tone_distribution", {})
	for required_key in recommended.skin_tone_distribution:
		if not saved_skin_tones.has(required_key):
			parsed["skin_tone_distribution"] = equalized_skin_tones()
			break
...
static func validate(settings: Dictionary) -> Dictionary:
	...
	_check_number(errors, driving, "camera_zoom_multiplier", 0.2, 2.0, "Driving camera zoom")
...
static func equalized_skin_tones() -> Dictionary:
	var equal_share := 100.0 / 7.0
	return {
		"very_light_percent": equal_share,
		"light_percent": equal_share,
		"medium_light_percent": equal_share,
		"medium_percent": equal_share,
		"medium_dark_percent": equal_share,
		"dark_percent": equal_share,
		"very_dark_percent": equal_share
	}
