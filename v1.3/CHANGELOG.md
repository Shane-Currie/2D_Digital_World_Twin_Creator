...
## 2026-09-21 — v1.3 completion checkpoint

- User declared **2D Digital World Twin Creator v1.3 complete**. Freeze this folder as the v1.3 release and continue future development in the separately named `v1.4` directory. Keep prior v1.1 and v1.2 releases as sibling folders on GitHub.
- This version adds map-derived movement-aware traffic queues, WASD walking, Shift-selected Reverse, 1 km/h cruise steps, configurable 0–100 km/h acceleration, numbered analog speedometer, per-town wagon odometer/trip meter, and OSM-derived roadside walking/crossing behaviour for NPCs and NPRs. The optional player camera rotation/soft guidance experiment was withdrawn after user feedback; it is **not** part of the release.
- Focused checks cover Creator settings, vehicle controls, traffic flow, crossings, odometer save/reset/visibility and Albury runtime presentation; the CLI/content suite passed 101 checks. These checks do not replace extended user driving, congestion or performance tests across uploaded towns.
- Known limitations remain: building/exterior authoring, interior design and local-LLM NPC conversations are later work; the Albury test fixture's saved aerial grid still spawns no NPDs; malformed or incomplete OSM exports and planet-scale/PBF streaming are not universally supported. This source release is not yet a self-contained Windows `.exe`.
