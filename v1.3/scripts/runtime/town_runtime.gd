...
func _ready() -> void:
	...
	population.setup(navigation_result.data, settings_result.data, projection, scale, town.cbd.bounds, str(town.id), town.map_bounds, features)
	...
	wagon.set_pedestrian_check(population.crossing_safety.car_may_move)
	camera = Camera2D.new()
...
func _build_hud() -> void:
	...
	speedometer_gauge.position = Vector2(420, 40)
	...
	odometer_panel.position = Vector2(420, 40)
