...
func update_detail_view(world_position: Vector2, camera_zoom: float, viewport_size: Vector2 = Vector2(640.0, 360.0)) -> void:
	# Drawing one CanvasItem for the entire imported city submits off-screen
	# geometry to the renderer. Keep a padded local view and only rebuild its
	# draw commands after the camera approaches that padding.
	if map_overview or camera_zoom <= 0.0:
		return
	var half_visible := viewport_size * 0.5 / camera_zoom
	var required := Rect2(world_position - half_visible, half_visible * 2.0).grow(64.0)
	if draw_view_bounds.has_area() and draw_view_bounds.encloses(required):
		return
	draw_view_bounds = required.grow(128.0)
	queue_redraw()


func _draws_bounds(bounds: Rect2) -> bool:
	return map_overview or not draw_view_bounds.has_area() or draw_view_bounds.intersects(bounds, true)


func _path_bounds(path: Dictionary) -> Rect2:
	return path.bounds if path.has("bounds") else _points_bounds(path.points)
...
func _draw() -> void:
	...
	for area in land_cover.areas:
		if not _draws_bounds(area.bounds):
			continue
	...
	for building in ground_buildings:
		if str(building.get("kind", "")) != "building" or not _draws_bounds(building.bounds):
			continue
		var outer: PackedVector2Array = building.outer
		if outer.size() < 3:
			continue
		var tags: Dictionary = building.tags
		var style_name := _building_style(tags, str(building.id))
		...
		_draw_styled_building(outer, palette, str(building.id))
	...
	for feature_value in features:
		...
		if roof.size() >= 3 and _draws_bounds(_points_bounds(roof)):
...
func _draw_water() -> void:
	...
	for feature_value in features:
		...
		if points.size() < 2 or not _draws_bounds(_points_bounds(points)):
			continue
...
func _draw_transport_paths(paths: Array) -> void:
	...
	for path_value in paths:
		...
		if points.size() < 2 or not _draws_bounds(_path_bounds(path)):
			continue
	...
	for path_value in paths:
		var path: Dictionary = path_value
		if bool(path.markings) and _draws_bounds(_path_bounds(path)):
...
func _draw_grass_stalks() -> void:
	...
	var grass_bounds := world_bounds if map_overview or not draw_view_bounds.has_area() else world_bounds.intersection(draw_view_bounds)
	if not grass_bounds.has_area():
		return
	var start_x := floorf(grass_bounds.position.x / spacing) * spacing
	var start_y := floorf(grass_bounds.position.y / spacing) * spacing
	var x := start_x
	while x <= grass_bounds.end.x:
		var y := start_y
		while y <= grass_bounds.end.y:
...
func _build_surface_areas() -> void:
	...
	for feature_value in features:
		...
		ground_buildings.append({"id": str(feature.get("id", "")), "kind": str(feature.get("kind", "")), "tags": feature.get("tags", {}), "outer": building_outer, "holes": building_holes, "bounds": _points_bounds(building_outer)})
...
func _build_roads() -> void:
	...
	for feature_value in features:
		...
		road_paths.append({
			...
			"points": points,
			"bounds": _points_bounds(points).grow(half_width + 8.0),
		...
		if not walkway and not bridge and not tunnel:
			...
			for side in RoadDimensionsScript.sidewalk_sides(tags):
				...
				sidewalk_paths.append({
					...
					"bounds": _points_bounds(points).grow(absf(offset) + sidewalk_width + 8.0),
