...
func build(features: Array, cbd_bounds: Dictionary, starting_location: Dictionary, game_settings: Dictionary = {}) -> Dictionary:
	...
	if int(pedestrian_graph.inferred_edge_count) > 0:
		warnings.append("%d pedestrian route directions use inferred roadside walking space because OSM does not specify a usable footpath. These are not surveyed pavements." % int(pedestrian_graph.inferred_edge_count))
...
func _build_graph(features: Array, mode: String, blocked_segments: Dictionary = {}) -> Dictionary:
	...
	for feature in features:
		...
		var explicit_sidewalk := tags.has("sidewalk") or tags.has("sidewalk:left") or tags.has("sidewalk:right") or tags.has("sidewalk:both")
		var inferred_walking := mode == "pedestrian" and highway not in PEDESTRIAN_SPECIFIC_HIGHWAYS and (not explicit_sidewalk or RoadDimensionsScript.sidewalk_sides(tags).is_empty())
		for index in range(points.size() - 1):
			...
			if one_way >= 0 and not edge_keys.has(forward_key):
				...
				if inferred_walking: inferred_edge_count += 1
