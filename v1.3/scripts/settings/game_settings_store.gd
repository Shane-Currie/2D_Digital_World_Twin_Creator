...
static func recommended_settings() -> Dictionary:
	...
	return {
		...
		"driving": {
			"max_speed_kmh": 200.0,
			"zero_to_hundred_seconds": 7.2,
			"reverse_max_speed_kmh": 20.0,
			"forward_speed": 108.0,
			"reverse_speed": 36.0,
			"acceleration": 42.0,
...
static func load_from_town(town_directory: String) -> Dictionary:
	...
	if not parsed.get("camera", {}) is Dictionary:
		...
		for key in recommended.camera:
			if not parsed["camera"].has(key):
				parsed["camera"][key] = recommended.camera[key]
	# Discard the short-lived camera/guidance experiment from saved v1.3 towns.
	# Reverse and all unrelated creator choices remain untouched.
	parsed["camera"].erase("rotate_while_walking")
	parsed["camera"].erase("rotate_while_driving")
	parsed.erase("guidance")
	...
	if not parsed.get("driving", {}) is Dictionary:
		...
		for key in recommended.driving:
			if not parsed["driving"].has(key):
				parsed["driving"][key] = recommended.driving[key]
...
static func validate(settings: Dictionary) -> Dictionary:
	...
	_check_number(errors, driving, "max_speed_kmh", 1.0, 400.0, "Maximum vehicle speed")
	if driving.has("zero_to_hundred_seconds"):
		_check_number(errors, driving, "zero_to_hundred_seconds", 3.0, 20.0, "0–100 km/h time")
	if driving.has("reverse_max_speed_kmh"):
		_check_number(errors, driving, "reverse_max_speed_kmh", 5.0, 40.0, "Maximum reverse speed")
