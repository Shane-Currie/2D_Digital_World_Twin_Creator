...
- This project is the completed **2D Digital World Twin Creator v1.3** release, based on v1.2. Follow the root rolling requirements log; preserve v1.1, v1.2 and this v1.3 checkpoint. New feature work belongs in the separate v1.4 directory.
...
- Building/place popups may state only tags attached directly to that footprint or clearly identified future creator overrides. Always show OpenStreetMap attribution and the source element ID; do not assign a nearby/contained tenant POI to an entire building without an explicit association.
