...
func _initialize() -> void:
	...
	assert(int(joined.data.pedestrian.inferred_edge_count) > 0 and str(joined.data.warnings).contains("not surveyed pavements"), "The creator did not disclose inferred roadside walking routes.")
