...
func _render_preview() -> void:
	...
	assert(imported.ok)
	assert(int(imported.statistics.inferred_clipped_water_areas) > 0)
	...
	var output_path := output_directory.path_join("the_rocks_environment.png")
	var save_error := viewport.get_texture().get_image().save_png(output_path)
	assert(save_error == OK)
	print("THE ROCKS ENVIRONMENT PREVIEW SAVED: ", output_path)
	quit(0)
